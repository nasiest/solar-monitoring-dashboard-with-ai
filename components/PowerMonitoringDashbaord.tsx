...
const PowerMonitoringDashboard = () => {
  ...
  // ✅ Only connect if backend exists (optional for deployment)
  useEffect(() => {
    ...
    try {
      ...
      socket.on('powerData', (data: any) => setPowerData(data));
      socket.on('predictedPower', (data: any) => setPredictedPower(data.predictedPower));

      return () => socket.disconnect();
    } catch (error) {
      console.warn('⚠️ Socket connection failed. Backend may not be running.');
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          {
            icon: <Zap className="text-amber-500 dark:text-amber-400 w-6 h-6" />,
            label: 'Current Power',
            value: powerData ? `${powerData.power} kW` : 'Loading...',
            border: 'border-yellow-200 dark:border-amber-700',
            text: 'text-amber-700 dark:text-amber-300',
          },
          {
            icon: <Sun className="text-orange-500 dark:text-orange-400 w-6 h-6" />,
            label: 'Predicted Power (AI)',
            value:
              predictedPower !== null
                ? `${predictedPower.toFixed(2)} kW`
                : 'Calculating...',
            border: 'border-orange-200 dark:border-orange-700',
            text: 'text-orange-600 dark:text-orange-300',
          },
          {
            icon: <Battery className="text-green-500 dark:text-emerald-400 w-6 h-6" />,
            label: 'Battery Level',
            value: `${batteryLevel}%`,
          ...
          {
            icon: <Leaf className="text-lime-600 dark:text-lime-400 w-6 h-6" />,
            label: 'CO₂ Saved',
            value: `${co2Savings} kg`,
